...
export function SiteFooter() {
  return (
    <footer className="w-full overflow-x-clip border-t border-[#17324d]/10 bg-[#17324d] text-white">
      <div className="mx-auto grid w-full min-w-0 max-w-[1440px] gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[1fr_auto] lg:px-10">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap lg:justify-end">
          {creditLinks.map(({ href, label, name, network }) => (
            <a
              aria-label={label}
              className="inline-flex min-h-11 min-w-0 items-center justify-between gap-3 rounded-[8px] border border-white/12 bg-white/8 px-3 text-sm font-black text-white transition hover:-translate-y-0.5 hover:bg-white/14 sm:justify-center"
              href={href}
              key={name}
              rel="noreferrer"
              target="_blank"
